...
class Dataset:
  ...
  def get_all_video_dirs(self):
    ...
    for dir_ in train_dirs:
      ...
      all_video_dirs[dir_] =  train_video_dirs
...
def main():
  ...
  all_dirs = dataset.get_all_video_dirs()
  print('keys ',all_dirs.keys())
  trainData = {}
  validData = {}
  for key,all_video_dirs in all_dirs.iteritems():
    ...
    trainData[key] = train_imdb

  # validData['a_decenter']['videos'] = list(chain.from_iterable(validData['a_decenter']['videos']))
  # validData['a']['videos'] = list(chain.from_iterable(validData['a']['videos']))
  
  
  # trainData['a_decenter']['videos'] = list(chain.from_iterable(trainData['a_decenter']['videos']))
  # trainData['a']['videos'] = list(chain.from_iterable(trainData['a']['videos']))
  
  with open(os.path.join(config.save_dir, 'train_imdb1.pickle'), 'wb') as f:
    pickle.dump(trainData, f)
  with open(os.path.join(config.save_dir, 'valid_imdb1.pickle'), 'wb') as f:
